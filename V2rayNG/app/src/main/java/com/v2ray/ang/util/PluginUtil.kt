...
//    }
    ...
    fun runPlugin(context: Context, config: ProfileItem?, domainPort: String?, finalPort: Int?) {
        Log.d(TAG, "runPlugin")

        if (config?.configType?.equals(EConfigType.HYSTERIA2) == true) {
            val configFile = genConfigHy2(context, config, domainPort, finalPort) ?: return
    ...
    fun realPingHy2(context: Context, config: ProfileItem?): Long {
        ...
        if (config?.configType?.equals(EConfigType.HYSTERIA2) == true) {
            ...
            val configFile = genConfigHy2(context, config, "0:${socksPort}", null) ?: return retFailure
            val cmd = genCmdHy2(context, configFile)
    ...
    private fun genConfigHy2(context: Context, config: ProfileItem, domainPort: String?, finalPort: Int?): File? {
        Log.d(TAG, "runPlugin $HYSTERIA2")

        val socksPort = domainPort?.split(":")?.last()
            .let { if (it.isNullOrEmpty()) return null else it.toInt() }
        val hy2Config = Hysteria2Fmt.toNativeConfig(config, socksPort, finalPort) ?: return null
