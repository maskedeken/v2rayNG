...
object V2rayConfigManager {
    ...
    private fun getV2rayNonCustomConfig(context: Context, config: ProfileItem): ConfigResult {
        ...
        result.finalPort = v2rayConfig.inbounds.find { it.protocol == "dokodemo-door" && it.tag == "map-in" }?.port
        return result
    }
...
//            }
    ...
    private fun outbounds(v2rayConfig: V2rayConfig, config: ProfileItem, isPlugin: Boolean): Pair<Boolean, String>? {
        if (isPlugin) {
            v2rayConfig.inbounds.add(V2rayConfig.InboundBean(
                tag = "map-in",
                port = Utils.findFreePort(listOf(0)),
                listen = LOOPBACK,
